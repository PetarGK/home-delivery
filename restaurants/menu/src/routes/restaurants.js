  ...
  try {
    ...
    if (ex.name === "ConditionalCheckFailedException")
      return res.status(400).json({
        message: `Restaurant with name: '${req.body.data.name}' already exist`,
      });
    else
      return res
        .status(500)
        .json({ message: `There is error while creating restaurant` });
  }
};

export const getRestaurant = async (req, res) => {
  const data = await req.namespace.models.restaurant.get(
    { name: req.params.id },
    {
  ...
  if (!data.Item) return res.status(404).json({});
